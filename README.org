...
| elgantt-elgantt-header-line-format             | See =header-line-format= for more information; warning that this variable is a mess                               |
** Examples 
...
*** Custom colors, etc.
The above examples customize the colors with the following macro, which will draw a gradient from the scheduled date to the deadline. See below for a description of how to use the =elgantt-create-display-rule= and =elgantt-create-action= macro. 
#+begin_src emacs-lisp :results silent
...
#+end_src
** Dark/light themes
Elgantt should automatically detect if you are using a dark or light theme and adjust the relevant faces accordingly. If it doesn’t, well, then it’s going to be a mess. 
** FAQ
*** Your code...
...
Seriously though, a lot of this code is bad. Some of it is probably pretty decent. I am not interested in changing any of it unless something does not work, or unless someone else does it for me. 
*** This is a mess/this is confusing
 This was an exercise in learning elisp and programming, and I realized a lot of things along the way. Mostly, that programming is not as much fun as I thought it was, and takes way more time than it should.

While I use this package daily to review my schedule because my job demands a lot of long-term scheduling, I don’t need the vast majority of the features, and suspect you don’t either. I tried to cover most reasonable use cases.
