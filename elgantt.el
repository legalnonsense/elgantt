...
(defun elgantt--parse-org-files ()
  ...
  (org-map-entries (lambda ()
		     ...
		     (let ((header (intern (elgantt--get-property-from-org-point (point)))))
		       ;; when using hashtags, find the tag with the hashtag, remove the hashtag, and intern it
		       (when elgantt-use-hashtags
			 (if (elgantt--get-hashtag-from-org-point (point))
			     (setq header (intern (elgantt--get-hashtag-from-org-point (point))))
			   (setq header nil)))
			 ...
			 (when header
			   (elgantt--add-properties header `(:category                  ,(elgantt--get-property-from-org-point (point))
								 ...
								 :calendar-date             ,(nth 2 (elgantt--get-calendar-data-from-org-point (point)))
								 :file                      ,(elgantt--get-file-from-org-point (point))
...
(defun elgantt--get-property-from-org-point (point)
  (cdr (car (org-entry-properties (point) "CATEGORY"))))

(defun elgantt--get-all-tags-as-string-from-org-point (point)
...
(defun elgantt--get-file-from-org-point (point)
...
(defun elgantt--get-deadline-from-org-point (point)
  (when (cdr (car (org-entry-properties (point) "DEADLINE")))
    (if (string= (cdr (car (org-entry-properties (point) "DEADLINE"))) (elgantt--get-property-from-org-point (point)))
	nil
...
(defun elgantt--get-active-time-range-from-org-point ()
  (when (cdar (org-entry-properties (point) "TIMESTAMP"))
    (cond ((string= (cdar (org-entry-properties (point) "TIMESTAMP")) (elgantt--get-property-from-org-point (point))) nil)
...
(defun elgantt--get-active-timestamp-from-org-point (point)
  (when (cdr (car (org-entry-properties (point) "TIMESTAMP")))
    (cond ((string= (cdr (car (org-entry-properties (point) "TIMESTAMP"))) (elgantt--get-property-from-org-point (point))) nil)
...
(defun elgantt--get-inactive-time-range-from-org-point ()
  (when (cdar (org-entry-properties (point) "TIMESTAMP_IA"))
    (cond ((string= (cdar (org-entry-properties (point) "TIMESTAMP_IA")) (elgantt--get-property-from-org-point (point))) nil)
...
(defun elgantt--get-inactive-timestamp-from-org-point (point)
  (when (cdr (car (org-entry-properties (point) "TIMESTAMP_IA")))
    (cond ((string= (cdr (car (org-entry-properties (point) "TIMESTAMP_IA"))) (elgantt--get-property-from-org-point (point))) nil)
...
(defun elgantt--get-headline-text-from-org-point (point)
  (cdr (car (org-entry-properties (point) "ITEM"))))

(defun elgantt--get-calendar-data-from-org-point (point)
  ...
  (let ((label nil)
	...
	(date nil))
    (dolist (tag (s-split ":" (elgantt--get-all-tags-as-string-from-org-point (point))))
	...
	(if (string= start-or-end-or-range "block")
	  ...
	  (cond ((elgantt--get-deadline-from-org-point (point)) (setq date (elgantt--get-deadline-from-org-point (point))))
		((elgantt--get-active-timestamp-from-org-point (point)) (setq date (elgantt--get-active-timestamp-from-org-point (point))))
		((elgantt--get-inactive-timestamp-from-org-point (point)) (setq date (elgantt--get-inactive-timestamp-from-org-point (point))))))))
