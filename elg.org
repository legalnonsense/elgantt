  ...
  (defun elg--add-year (year)
    (if elg::date-range
        (when (not (memq year elg::date-range))
          (cond ((< year (first elg::date-range))
...
#+begin_src emacs-lisp :results silent
  ...
  are already present. Updates the cell's display."
    (mapc (lambda (date) 
            ...
            (let ((year (string-to-number (plist-get props :elg-date))))
              (unless (memq year elg::date-range)
                (elg--add-year year)))
...
#+RESULTS:
: elg--action-rule-turn-this-shit-red
